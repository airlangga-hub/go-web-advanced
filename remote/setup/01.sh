...
read -p "Enter password for PostgreSQL airlangga DB user: " DB_PASSWORD
...
curl -L https://github.com/pressly/goose/releases/download/v3.26.0/goose_linux_x86_64.tar.gz | tar xvz
...
sudo -i -u postgres psql -c "CREATE DATABASE go-web"
sudo -i -u postgres psql -d go-web -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d go-web -c "CREATE ROLE airlangga WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the greenlight database to the system-wide environment
# variables in the /etc/environment file.
echo "DB_DSN='postgres://airlangga:${DB_PASSWORD}@localhost/go-web'" >> /etc/environment
